...
An API Wallet signing for several accounts shares its nonce state, which can cause conflicts. Owly requires separate API Wallets; this does not restrict a key to one sub-account at the protocol level. See [Account Type](/guide/account-types).
